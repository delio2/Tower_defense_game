...
### D35 — The wave view follows the fight 🟢 *(user decision, 2026-09-22)*
- **Question:** the side-by-side sheet of 2.5-E2 showed the gap: the approved mood shot frames the flower at about 40% of the screen width, while task A2 asks the wave view to reach the outer ring (9.0), which leaves the flower at about 20% and the enemies tiny. The mood shot predates D32, which turned the rings into range bands.
...
- **Consequence:** a new weight or icon is one menu item or one script run away, and no colour or typeface is written by hand in `Theme.uss`.
