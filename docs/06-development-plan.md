...
> Version 3.1 · 2026-09-22 · Status: active (3.1: Phase 2.5 progress — quality levels, bot farm and emulator benchmark, HUD split, EN + IT, playtest kit). v2 inserted **Phase 2.5 — Visual overhaul** (D30); v3 details Phase 2.5 and Phase 3 as task lists with estimates and "done" criteria, and adopts **vertical slices on shared foundations** (D33). The month-by-month calendar is in `10` §4; this document defines the **phases, deliverables and exit gates**.
...
| A3 | **Light and post:** warm key `#FFE2BC` from the upper left, gradient ambient, soft shadows (medium/high), post volume (bloom 1.1 / 0.3, vignette 0.25, tone mapping None); quality levels Low (no shadows, no bloom) / Medium / High | side-by-side with `docs/art/moodshot-wave.png`; Low level ≥ 60 fps on the Pixel — 🔄 key light, hemisphere ambient, bloom/vignette/tone mapping None done (`ArenaLighting`); shadows on both URP assets (1 cascade, 45 u). ✅ **2026-09-22: Low/Medium/High split** (D36) — three URP assets on the mobile renderer (Low: no shadows, no HDR, post skipped, scale 0.75 · Medium: the former Mobile asset · High: soft shadows 2048, 4× MSAA, scale 1.0), the PC level removed, option *Graphics: Auto/Low/Medium/High* switching at runtime (Auto by device memory). In the editor Low cuts SetPass calls 24 → 6 and triangles ~18k → ~9.7k. **Left:** the fps check on devices (High on the Pixel, Low on a 4 GB phone) |
| A4 | **Three-surface shader** (Shader Graph, URP Lit): body + fresnel rim (category colour) + emissive; materials Core, petal, teal, gold, mint, coral, rose, ally as assets in `Resources/Materials`; SRP Batcher compatible | ≤ 40 draw calls with 60 enemies; works in a player build (no stripping) — 🔄 hand-written URP shader `TowerDefense/ThreeSurface` (lit body, fresnel rim, emissive; ShadowCaster + DepthOnly), one material per style, assets in `Resources/Materials`; works in the device build (E1). **Draw calls (2026-09-22, editor, wave 17 with 6 enemies alive):** ~20–23 draws in 2–4 SRP batches; the shop with its cards ~68 (UI Toolkit included). Every enemy is one draw (plus one shadow draw on Medium/High), so 60 enemies will be **above the 40-draw budget** (estimate ~75 on Low, ~130 on Medium/High) even though the SRP Batcher keeps them in a handful of batches. Lever if a device misses its fps target: GPU instancing for enemies (one draw per enemy kind). To measure properly with the autoplay benchmark (next step) |
...
| A6 | **Model pipeline:** Blender family script v1 generating the **14 existing modules** and **7 enemies + Guardian** (from the `11` silhouettes), exported to `Assets/Content/Resources/Models/`; prefab per model; `ModuleAsset`/`EnemyAsset` reference a prefab; `PrimitiveMesh` fallback | every current module/enemy shows its own silhouette in game — ✅ 2026-09-22: `Tools/blender/build_models_v1.py` builds the 21 models (`Art/Blender/models_v1.blend`) and exports FBX to `Resources/Models/{Modules,Enemies}`; the game loads them by kind (`ArenaKit.CreateModel`, sub-mesh 1 = glowing accent) with the primitive fallback; modules face outward, shards point at the Core with a slow roll. Star faces (★★/★★★) come with the v2 family scripts (3b) |
...
| E3 | **Testers (3–5, muted):** Gate 1 + Gate 2 questions + "calm but attractive, or prototype?" on the new build. 🔄 **Kit ready (2026-09-22):** protocol [`13`](13-playtest-protocol.md) (who, setup, what to say, observation sheet, questions); the game logs every session (`LocalAnalytics` + `SessionRecorder`: real wave and shop seconds, longest idle per shop, run length, the first-run funnel); `python Tools/playtest/report.py` pulls the logs over USB and checks them against the Gate targets; *TowerDefense → Build → Android tester APK* (release). **Left: the sessions themselves (user)** |
...
- [x] Localization: key-based strings, **EN + IT** — done early (2026-09-22, D38): `Strings.txt` with 129 keys, `Loc`, Language option, a table test; checked in Play mode in both languages.
...
**Status (2026-09-22):** Phase 0 ✅ · Phase 1 ✅ (Gate 1 with testers pending) · Phase 2 ✅ in code (Gate 2 with testers pending) · **Phase 2.5 🔄** (design ✅; foundations A1–A7 ✅ (quality levels Low/Medium/High done, D36) except the device checks of A3/A4; slices **B, C, D ✅**; E1 build ✅ on a Pixel 10, E2 ✅; bot farm and autoplay benchmark ✅ (D37); HUD split ✅; EN + IT strings ✅ (D38); playtest kit ✅ (`13`); left: the E3 tester sessions and the device fps checks) · Phase 3 ⬜ (waits for approval of `11`–`12`). 52 automated tests, balance `0.3.0`.

**Done in order:** bot runner → ScriptableObject content → `BuySlot` → compact notation → UI Toolkit HUD → magnetic drag with previews → merge animation and wave summary → options and haptics → 14 modules, 7 enemies, 3 acts, elites → balance v0.3 → Core types, Grades 1–3, Endless → replay R2 → save/resume → Core tuning, enemy silhouettes → UI foundations (tokens, typefaces, icons) → wave slice → shop slice → overlays → Android build and device pass → quality levels (D36) → bot farm with archetype and lookahead bots, autoplay benchmark on emulators (D37) → HUD split into panels → keyed strings EN + IT (D38) → playtest kit (`13`, session logs, report script).

**Next, in order:**
1. ✅ **Phase 2.5-A foundations** (A1 view split · A2 camera · A3 light, post and quality levels (D36) · A5 sky and
   rings · A6 models · A7 tokens, typefaces, icons). Left inside A3/A4: fps on devices per level, a draw-call count
   with 60 enemies.
...
4. 🔄 **2.5-E3: the testers** (3–5, muted) — Gate 1, Gate 2 and Gate 2.5 together. The kit is ready (`13`, tester APK,
   `Tools/playtest/report.py`); the sessions are the user's.
5. ⬜ In parallel, the user: approve `11` (names, gates, economy) and choose the launch scope of `12`; open the
   **Google Play account** (D13).
6. ⬜ **Phase 3a** in the order of its table; then 3b art completion and 3c FTUE, audio, profiling (EN + IT strings are
   already done, D38).
7. ⬜ Phase 4: Garden (Archive), Daily/Weekly with local leaderboard, replay codes and ghosts, the remaining `Services`
   fakes (analytics already has one: `IAnalytics` → `LocalAnalytics`).

**Resume here (handoff, 2026-09-22):**
- *Waiting on the user:* (1) approval of `11` names and systems and the launch scope of `12` — Phase 3 does not start before; (2) Google Play account; (3) **the E3 tester sessions** (Gates 1, 2 and 2.5), following `13` with `Builds/Android/TowerDefense-tester.apk`; (4) confirm the Italian word choices (D38) and the balance proposals from the bot farm (D37).
- *Next task:* **E3, the testers** — follow `13`; APK `Builds/Android/TowerDefense-tester.apk`; after each tester `python Tools/playtest/report.py`; and the device check of the quality levels (High on the Pixel, Low on a 4 GB phone). After that, Gate 2.5 and Phase 3.
- *Automated testing (D37):* `dotnet run -c Release --project Tools/BotFarm -- --seeds 1000 --strategies all [--core Bastion] [--grade 2]` plays thousands of bot runs in seconds (CSV + summary in `Temp/Balance/`). For the device side: Unity menu *TowerDefense → Build → Android benchmark APK* (ARM64, development; the emulators translate ARM), then `python Tools/bench/run_emulators.py` (all AVDs in parallel, headless; `--device <serial>` for a real phone) → `Temp/Bench/<time>/summary.md`, logs and screenshots per device. Emulator fps are not phone fps. The game takes its own screenshots of three shops and the three Guardian waves; the script pulls them. Emulator traps (handled): the one-time "Viewing full screen" and tablet taskbar prompts steal focus (tapped away via `uiautomator`), a foldable needs `screencap -d <display>`, the images' `uwb-service` crashes are noise.
...
- *Where things are:* design system (tokens, icons, all screen mockups) — artifact linked in `docs/README.md`; tokens copy `Tools/design/tokens.json` → `python Tools/tokens_to_uss.py` regenerates `Theme.uss` (including `.theme-contrast` and `.text-large`); fonts → *TowerDefense → Content → Build font assets* (`Assets/Scripts/Editor/FontMenu.cs`); icons → `python Tools/icons_to_png.py`; models → run `Tools/blender/build_models_v1.py` in Blender; arena code in `Assets/Scripts/Presentation/Arena/`, HUD in `UI/` (a `HudView` facade over one panel per screen part).
- *Working notes:* a **dynamic font asset rewrites itself** whenever Play mode meets a character its atlas does not hold, which shows up as a 2 MB diff on `Assets/UI/Fonts/*.asset`. It is noise: `git checkout -- Assets/UI/Fonts/` before committing, or rebuild them from the menu. Portrait captures = render the camera into a 1080 × 1920 render texture **and** set `PanelSettings.targetTexture`, then read the pixels in a **second** call — the panel needs a frame to repaint (always restore both to null afterwards). `Time.timeScale = 0` freezes anything driven by `Time.time`, which is how a 2.5 s bubble can be captured at all. Unity sometimes drops `InputSystem_Actions` from `preloadedAssets` in `ProjectSettings.asset` after Play mode — revert that line. USS reloads during Play leave a stale HUD in the editor (restart Play). When capturing overlays from `execute_code`: the runner re-applies the pause every frame (set its `_paused` field instead of calling `SetPaused`), and the wave summary closes after 2.2 s (freeze with `Time.timeScale = 0`). A script edit during Play recompiles and restarts the run. A long `execute_code` (an APK build) times out on the MCP side but keeps running in Unity.

**Known open points:** the Grade ladder is steep between 1 and 2 (confirmed by the farm); the Sniper archetype passes 85% with Bastion and economy builds are weak (D37 proposals, for the user); Mortar and Echo rarely appear (rare + act 2); no emulator runs Android 8 (API 26), so the minimum version is untested; the "Draw Calls Count" counter reads 0 in players (SetPass calls are logged instead); damage numbers stack into an unreadable cluster around a Guardian hit by several modules (seen in the emulator shots of wave 12: merge or offset repeated numbers); the state hash gained fields (slot count, elites, dash timing) under balance version 0.3.0 — no replays from 0.2.0 exist.
